...
public class Morphology {
	...
	public BufferedImage boundaryDetection(){
		...
		BufferedImage imageBounded = new BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);
	...
	private boolean isPixelOfBound(int i, int j) {
		...
		for(int k= i-ee ; k<= i+ee ; k++){
			for(int w = j-ee ; w<= j+ee ; w++){
				if(inside(k,w)){
					if(image.getRGB(k,w)==Color.white.getRGB() || (image.getRGB(k,w)==(Color.white.getRGB() ^1) ) )white = true;
					else if (image.getRGB(k, w)==Color.black.getRGB() ||  image.getRGB(k, w)== (Color.black.getRGB() | 1) ) black = true;
