...
public class RSA {  
	...
	public static boolean check(int p, int q){
		...
		if(a.multiply(b).compareTo(new BigInteger(""+Integer.MAX_VALUE))>0 ){
			JOptionPane.showMessageDialog(null, "p*q excede el tamaño de un entero de 32 bits: "+Integer.MAX_VALUE);
			return false;
		}
		if(a.multiply(b).compareTo(new BigInteger(""+(int)Character.MAX_VALUE))<0 ){
			JOptionPane.showMessageDialog(null, "p*q no es suficiente para cubrir el Codigo ASCII\np*q = " + (p*q)+
					", pero el valor minimo exigido es: "+(int)Character.MAX_VALUE);
			return false;
		}
		return true;
	}
	...
	public String toString(){
		return ("N: "+this.N+" P: "+this.p  + " q: "+this.q +" d: "+this.d+ " e: "+this.e);
	}
	
	public int encrypt(int x){
		BigInteger encripted = new BigInteger(x+"");
		encripted = encripted.modPow(new BigInteger(e+""), new BigInteger(N+"") );
	...
	public static void main(String[] args) {
		...
		for(int i=0 ; i<s.length() ; i++){
			System.out.println("Char ASCII: "+ (int)s.charAt(i) +" Encripted: "+rsa.encrypt((int)s.charAt(i)));
		}
